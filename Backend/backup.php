...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if ($connectionDetails) {
            ...
            $password = decryptPassword($encryptedPassword); // αποκρυπτογραφηση MySQL κωδικου

            
            $encryptedSshPassword = $connectionDetails['ssh_password'];
            $sshPassword = decryptPassword($encryptedSshPassword); // αποκρυπτογραφιση SSH κωδικου
            ...
            // δημιουργια φακελου χρηστη με ονομα απο το email του
            $userFolder = 'backups/' . preg_replace('/[^a-zA-Z0-9]/', '_', $email);
            if (!is_dir($userFolder)) {
                mkdir($userFolder, 0777, true);
            }

            // δημιουργια φακελου συνδεσης μεσα στον φακελο του χρηστη
            ...
            // δημιουργια ονοματος αντιγραφου ασφαλειας μεσα στον φακελο
